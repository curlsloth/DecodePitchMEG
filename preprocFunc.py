...
# def seq_mlm(data, n_perm, corr_all):
#     import pandas as pd
#     import statsmodels.formula.api as smf
...
#                     df = {'sub': n0, 'pitchDist': (ploc-n1)*2, 'samePitch': (ploc-n1==3 or ploc-n1==6), 'auc': score_pitch_ind[n0,n1,ploc], 'coch': corr_all[n1,ploc]}
...
#         md = smf.mixedlm('auc ~ coch + pitchDist + samePitch + samePitch*pitchDist', data=pitchDist, groups=pitchDist['sub'], re_formula='~pitchDist')
     
#         try: 
#             mdf_list.append(md.fit(disp=False))
#         except: # very few cases it will get singular
#             import random
#             for nRow in range(len(pitchDist)):
#                 pitchDist['auc'].iloc[nRow] += (random.random()-0.5)*0.0002 # if getting singular, add 0.001 jitter to the data 
...
#     df_tvalues = pd.DataFrame()
#     for m in mdf_list:
#         df_tvalues = pd.concat([df_tvalues, pd.DataFrame([m.tvalues[:5]])], ignore_index=True) 
